...
class Form(QtWidgets.QWidget, Ui_Form):
    ...
    def do_explode(self):
        ...
        logger.debug("[ExplodeGeometry] --- Explode triggered ---")
        logger.debug("[ExplodeGeometry] Mode:            %s", mode)
        logger.debug("[ExplodeGeometry] Add Shell:        %s (offset=%s)", addShell, shell_amount)
        logger.debug("[ExplodeGeometry] Add Edit Mesh:    %s", addEditMesh)
        logger.debug("[ExplodeGeometry] Collapse Stack:   %s", collapseNode)
        logger.debug("[ExplodeGeometry] Center Pivot:     %s", centerPivot)
        logger.debug("[ExplodeGeometry] Delete Original:  %s", deleteOriginal)
        logger.debug("[ExplodeGeometry] Selected objects: %d", len(rt.selection))

        if len(rt.selection) <= 0:
            msg = "Nothing selected. Please select a node to explode."
            logger.warning("[ExplodeGeometry] ERROR: %s", msg)
            self.show_alert(msg)
        else:
            with pymxs.undo(True, "Explode Geometry"):
                if self.trimesh_option.isChecked():
                    transformation_set = list(rt.selection)
                    logger.debug("[ExplodeGeometry] [TriMesh] Processing %d object(s)", len(transformation_set))
                    for i, n in enumerate(transformation_set, 1):
                        logger.debug("[ExplodeGeometry] [TriMesh] Object %d/%d: '%s'", i, len(transformation_set), n.name)
                        convert_to_triangle_faces(n, addShell, shell_amount,
                                                  addEditMesh, collapseNode,
                                                  centerPivot)
                        if deleteOriginal:
                            logger.debug("[ExplodeGeometry] [TriMesh] Deleting original '%s'", n.name)
                            rt.delete(n)

                if self.mnmesh_option.isChecked():
                    transformation_set = list(rt.selection)
                    logger.debug("[ExplodeGeometry] [MNMesh] Processing %d object(s)", len(transformation_set))
                    for i, node in enumerate(transformation_set, 1):
                        logger.debug("[ExplodeGeometry] [MNMesh] Object %d/%d: '%s'", i, len(transformation_set), node.name)
                        convert_to_mnmesh_faces(node, addShell, shell_amount,
                                                addEditMesh, collapseNode,
                                                centerPivot)
                        if deleteOriginal:
                            logger.debug("[ExplodeGeometry] [MNMesh] Deleting original '%s'", node.name)
                            rt.delete(node)

            logger.info("[ExplodeGeometry] Done. Redrawing views.")
            rt.redrawViews()
